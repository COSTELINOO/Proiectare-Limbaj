...
class SymTable {
    ...
    for (int i = 0; i <
      ...
      if (var [i].nume == n) {
        if (var [i].size - 1 < index) {
          ...
          cout << "Eroare la linia " << yylineno << ": indexul \"" << index << "\"  este prea mare (209)\n";
          errorCount++;
  ...
  void addScope(SymTable * scope, vector < variabile > paramss = vector < variabile > ()) {
    ...
    if ((existsScope(aux) == false) || (
      ...
      cout << "Eroare la linia " << yylineno << ": tabela cu numele \"" << scope -> nume << "\" si scopul: " << scope -> tip_scop << " este deja definita(209)\n";
  ...
  void addVariabila(char * t, char * n, int dim = 0,
    ...
    if (isDefined(name.c_str()) == false) {
      ...
      //cout << "Eroare la linia " << yylineno << ": variabila \"" << name << ' ' << tip_scop << "\" exista deja(210)\n";
      //errorCount++;
  ...
  void addParam(char * t, char * n, int dim = 0) {
    ...
    if (existsVariabila(name.c_str()) == false) {
      ...
      cout << "Eroare la linia " << yylineno << ": parametrul \"" << name << "\" exista deja in acest scop(211)\n";
